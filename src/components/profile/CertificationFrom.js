  ...
  const fetchCertificationData = async () => {
    try {
      ...
      if (response.code === 200) {
        setCertificationData(response.data.certification_data);
      } else {
        console.error('Error fetching certification data:', response.message);
      }
    } catch (error) {
      console.error('Error:', error);
    }
  };
  useEffect(() => { fetchCertificationData(); }, []);
...
const handleSubmit_working = async (e) => {
  ...
  try {
    ...
    const response = await fetchData('add-student-certification', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (response.code === 200) {
      ...
      console.error('Error saving certification:', response.message);
      setSuccessMessage('⚠️ Failed to save certification.');
    }
  } catch (error) {
    console.error('Error:', error);
...
const handleSubmit_new = async (e) => {
  ...
  try {
    ...
    const res = await fetchData('add-student-certification', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(payload),
    });
  ...
  const handleEdit = (index) => {
    const data = certificationData[index];
    setFormData({
      id: data.id,
      userId,
  ...
  const handleDelete = async (certificationId) => {
    try {
      const response = await fetchData('delete-certification', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId, certificationId }),
      });
      if (response.code === 200) fetchCertificationData();
      else console.error('Error deleting certification:', response.message);
      ...
      {formVisible && editingIndex === null && (
        <form onSubmit={handleSubmit} className='p-3 mb-2'  style={{border:'1px solid #bcbcbc',borderRadius:"15px"}}>
          <div className='d-flex form-row'>
            <label>Degree</label>
            <input type="text" name="degree" placeholder="Degree" value={formData.degree} onChange={handleChange} className='form-control' />
          </div>
          ...
          >
            <div className='d-flex align-items-start gap-3 p-2'>
              ...
              <div className='list_icon flex-shrink-0'>
                ...
                {/* <img src={icons['CertificationCapRectangle.svg']} alt="Icon" style={{ width: '40px', height: '40px', objectFit: 'cover' }} /> */}
              </div>
              ...
              <div className='d-flex flex-column gap-1'>
                ...
                <button className="btn btn-sm text-danger  text-sm" onClick={() => handleDelete(edu.id)}>
                  <Trash2 />
                </button>
              </div>
            </div>
